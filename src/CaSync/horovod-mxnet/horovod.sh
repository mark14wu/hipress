...
mpirun -np $1 \
    -H $2 \
	-x NCCL_DEBUG=INFO -x LD_LIBRARY_PATH -x PATH -x NCCL_IB_DISABLE=1 -x NCCL_SOCKET_IFNAME=ens14f1 -x NCCL_TREE_THRESHOLD=0 -x MXNET_NUM_OF_PARTICIPANTS=$1 -x MXNET_EXEC_BULK_EXEC_MAX_NODE_TRAIN=$9 -x HOROVOD_LOG_LEVEL="fatal" \
	\
