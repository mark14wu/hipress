...
MPI_LIB = _load_library('mpi_lib' + get_ext_suffix(),
                        ...
                        'HorovodGather', 'HorovodCBroadcast'])
...
def gather(tensor, root_rank, batch_id, name=None):
    if name is None and not _executing_eagerly():
        name = 'HorovodGather_%s' % _normalize_name(tensor.name)
    return MPI_LIB.horovod_gather(tensor, root_rank=root_rank,
        batch_id=batch_id, name=name)

def cbroadcast(tensor, root_rank, batch_id, name=None):
    if name is None and not _executing_eagerly():
        name = 'HorovodCBroadcast_%s' % _normalize_name(tensor.name)
    return MPI_LIB.horovod_c_broadcast(tensor, root_rank=root_rank,
        batch_id=batch_id, name=name)
