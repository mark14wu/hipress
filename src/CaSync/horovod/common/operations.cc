...
OperationManager* CreateOperationManager(HorovodGlobalState& state) {
  ...
  std::vector<std::shared_ptr<AllgatherOp>> balltoall_ops;
  ...
  allreduce_ops.push_back(std::shared_ptr<AllreduceOp>(new MPI_CUDAAllreduce(&mpi_context, &cuda_context, &state)));
    ...
    allgather_ops.push_back(std::shared_ptr<AllgatherOp>(
        new NCCLAllgather(&nccl_context, &mpi_context, &cuda_context, &state)));
  ...
  gatherBcast_ops.push_back(std::shared_ptr<GatherBcastOp>(new MPIGatherBcast(&mpi_context, &state)));
  gather_ops.push_back(std::shared_ptr<GatherOp>(new MPIGather(&mpi_context, &state)));
  alltoall_ops.push_back(std::shared_ptr<AlltoallOp>(new MPIAlltoall(&mpi_context, &state)));
  balltoall_ops.push_back(std::shared_ptr<AllgatherOp>(new MPIBAlltoall(&mpi_context, &state)));
  std::shared_ptr<ErrorOp> error_op(new ErrorOp(&state));

  return new OperationManager(&state.param_manager, allreduce_ops, allgather_ops,
                              broadcast_ops, gatherBcast_ops, gather_ops, alltoall_ops, balltoall_ops, error_op);
}
...
Response ConstructResponse(std::unique_ptr<MessageTable>& message_table,
  ...
  // If we are doing an allreduce or broadcast or gatherbcast, check that all tensor shapes are
  // identical.
  if (message_type == Request::ALLREDUCE ||
      message_type == Request::BROADCAST ||
      message_type == Request::CBROADCAST ||
      message_type == Request::GATHERBCAST) {
  ...
  // If we are doing a broadcast or gatherbcast or gather, check that all root ranks are identical.
  ...
  } else if (message_type == Request::CBROADCAST) {
    response.set_response_type(Response::CBROADCAST);
  } else if (message_type == Request::GATHERBCAST) {
    response.set_response_type(Response::GATHERBCAST);
  } else if (message_type == Request::GATHER) {
    response.set_response_type(Response::GATHER);
    for (auto dim : tensor_sizes) {
      response.add_tensor_size(dim);
    }
  }
...
int64_t TensorFusionThresholdBytesCPU() {
  if (horovod_global.cpu_fusion_buffer_size > 0)
    return horovod_global.param_manager.TensorFusionThresholdBytes();
  return horovod_global.cpu_fusion_buffer_size;
}
  ...
  entries.reserve(response.tensor_names().size());
  {
    ...
    for (auto& name : response.tensor_names()) {
      ...
      assert(response.response_type() == Response::ALLREDUCE   ||
             ...
             response.response_type() == Response::CBROADCAST  ||
             response.response_type() == Response::GATHER      ||
             response.response_type() == Response::GATHERBCAST ||
             response.response_type() == Response::ALLTOALL    ||
             response.response_type() == Response::BALLTOALL  ||
...
void BackgroundThreadLoop(HorovodGlobalState& state, MPIContext& ctx) {
  ...
  // Open the breakdown timestamp on all nodes
  auto horovod_timestamp = std::getenv(HOROVOD_TIMESTAMP);
  if (horovod_timestamp != nullptr) {
    state.timestamp.Initialize(std::string(horovod_timestamp),
                               static_cast<unsigned int>(size),
  ...
  if (is_coordinator) {
    ...
    state.partitioned_msg_table = std::unique_ptr<PartMessageTable>(new PartMessageTable());
  }
...
bool RunLoopOnce(HorovodGlobalState& state, MPIContext& ctx, bool is_coordinator) {
  ...
  std::queue<Request> message_queue;
  {
    ...
    while (!state.message_queue.empty()) {
      ...
      // if (message.request_type() == Request::GATHER) {
      //   EnqueueTimestamp(message.tensor_name(), "SGATHER", std::to_string(message.request_batchid()));
      // } else if (message.request_type() == Request::BROADCAST) {
      //   EnqueueTimestamp(message.tensor_name(), "SBCAST", std::to_string(message.request_batchid()));
      // }
    }
  }
  ...
  if (is_coordinator) {
    while (!message_queue.empty()) {
      ...
      if (reduce) {
        ...
        if (found != std::string::npos) { // is the partitioned request
          auto header_name = name.substr(0, found);
          auto table_iter = state.partitioned_msg_table->find(header_name);
          if (table_iter == state.partitioned_msg_table->end()) {
            std::vector<std::string> messages = {name};
            messages.reserve(static_cast<unsigned long>(state.size));
            state.partitioned_msg_table->emplace(header_name, std::move(messages));
          } else {
            std::vector<std::string>& messages = table_iter->second;
            messages.push_back(name);
            int count = (int)messages.size();
            if (count == state.size) {
              for (auto& part_name : messages) {
                ready_to_reduce.push_back(part_name);
              }
              state.partitioned_msg_table->erase(table_iter);
            }
          }
        } else { // the normal request
    ...
    for (int i = 1; i < state.size; ++i) {
      ...
      for (auto& received_message : received_message_list.requests()) {
        ...
        if (reduce) {
          auto name = received_message.tensor_name();
          size_t found = name.find_last_of('+');
          if (found != std::string::npos) { // is the partitioned request
            auto header_name = name.substr(0, found);
            auto table_iter = state.partitioned_msg_table->find(header_name);
            ...
            ready_to_reduce.push_back(received_message.tensor_name());
          }
        }
      }
    ...
    // construct response_list from responses
    // can be fused responses will be fused as a response
    ResponseList response_list;
    response_list.set_shutdown(should_shut_down);
    {
      // Protect access to tensor table.
      std::lock_guard<std::mutex> guard(horovod_global.mutex);
      Response ar_response, ag_response; // allreduce, allgather, gather, bcast reponse
      DataType ar_dtype, ag_dtype;
      int64_t ar_tensor_size, ag_tensor_size;
      std::vector<Response> g_responses(horovod_global.size);
      std::vector<DataType> g_dtypes(horovod_global.size);
      std::vector<int64_t> g_tensor_sizes(horovod_global.size);
      std::vector<int32_t> g_root_ranks;
      int32_t gather_count = 0;

      std::vector<Response> b_responses(horovod_global.size);
      std::vector<DataType> b_dtypes(horovod_global.size);
      std::vector<int64_t> b_tensor_sizes(horovod_global.size);
      std::vector<int32_t> b_root_ranks;
      int32_t cbcast_count = 0;
      ...
      while (!responses.empty()) {
        ...
        if (response.response_type() == Response::ResponseType::ALLREDUCE) {
          ...
          if (ag_response.tensor_names().size() == 0) {
            ...
            ag_tensor_size = TotalByteSizeOfAllgatherOutput(response.tensor_sizes(), entry, ctx);
          } else {
            int64_t new_allgather_byte_size = TotalByteSizeOfAllgatherOutput(response.tensor_sizes(), entry, ctx);
        ...
        } else if (response.response_type() == Response::ResponseType::GATHER) {
          auto& g_response = g_responses[response.root_rank(0)];
          auto& g_tensor_size = g_tensor_sizes[response.root_rank(0)];
          auto& g_dtype = g_dtypes[response.root_rank(0)];
          if (g_response.tensor_names().size() == 0) {
            g_response.set_response(response);
            g_dtype = entry.tensor->dtype();
            g_tensor_size = TotalByteSizeOfAllgatherOutput(response.tensor_sizes(), entry, ctx); // the same with allgather
            g_root_ranks.push_back(response.root_rank(0));
          } else {
            int64_t new_gather_byte_size = TotalByteSizeOfAllgatherOutput(response.tensor_sizes(), entry, ctx);
            if (response.response_type() == g_response.response_type() &&
                response.devices() == g_response.devices() &&
                entry.tensor->dtype() == g_dtype &&
                g_tensor_size + new_gather_byte_size <= TensorFusionThresholdBytesCPU()) {
              g_tensor_size += new_gather_byte_size;
              g_response.add_allgather_response(response);
            } else {
              gather_count += 1;
              ...
              g_response.set_response(response);
              g_dtype = entry.tensor->dtype();
              g_tensor_size = TotalByteSizeOfAllgatherOutput(response.tensor_sizes(), entry, ctx);
            }
          }
        } else if (response.response_type() == Response::ResponseType::CBROADCAST) {
          int element_size = ctx.GetMPITypeSize(entry.tensor->dtype());
          auto& b_response = b_responses[response.root_rank(0)];
          auto& b_dtype = b_dtypes[response.root_rank(0)];
          auto& b_tensor_size = b_tensor_sizes[response.root_rank(0)];
          if (b_response.tensor_names().size() == 0) {
            b_response.set_response(response);
            b_dtype = entry.tensor->dtype();
            if (entry.num_elem > 0) {
              b_tensor_size = entry.num_elem * element_size;
            } else {
              b_tensor_size = entry.tensor->size();
            }
            b_root_ranks.push_back(response.root_rank(0));
          } else {
            int64_t new_bcast_byte_size;
            if (entry.num_elem > 0) {
              new_bcast_byte_size = entry.num_elem * element_size;
            } else {
              new_bcast_byte_size = entry.tensor->size();
            }
            if (response.response_type() == b_response.response_type() &&
                response.devices() == b_response.devices() &&
                entry.tensor->dtype() == b_dtype &&
                b_tensor_size + new_bcast_byte_size <= TensorFusionThresholdBytesCPU()) {
              b_tensor_size += new_bcast_byte_size;
              b_response.add_tensor_name(response.tensor_names()[0]);
            } else {
              cbcast_count += 1;
      ...
      assert(gather_count == 0); // the simplest case
      if (g_root_ranks.size() > 0) {
        if (g_root_ranks.size() == 1) { // original gather operator
          response_list.add_response(g_responses[g_root_ranks[0]]);
        } else { // regenerate a alltoall operator
          Response alltoall_response;
          std::sort(g_root_ranks.begin(), g_root_ranks.end());
          alltoall_response.set_response(g_responses[g_root_ranks[0]]);
          alltoall_response.add_entry_count(g_responses[g_root_ranks[0]].tensor_names().size());
          for (int i = 1; i < g_root_ranks.size(); i++) {
            alltoall_response.add_alltoall_response(g_responses[g_root_ranks[i]]);
          }
          alltoall_response.set_response_type(Response::ALLTOALL);
          alltoall_response.set_entries_offsets();
          assert(alltoall_response.entries_counts().size() == g_root_ranks.size());
          assert(alltoall_response.root_ranks().size() == g_root_ranks.size());
          response_list.add_response(alltoall_response);
        }
      }
      // for (auto &response : g_responses) {
      //   if (response.tensor_names().size() > 0) {
      //     response_list.add_response(response);
      //   }
      // }
      assert(cbcast_count == 0);
      if (b_root_ranks.size() > 0) {
        if (b_root_ranks.size() == 1) { // original broadcast operator
          response_list.add_response(b_responses[b_root_ranks[0]]);
        } else { // regenerate a allgather operator
          Response allgather_response;
          std::sort(b_root_ranks.begin(), b_root_ranks.end());
          allgather_response.set_response(b_responses[b_root_ranks[0]]);
          allgather_response.add_entry_count(b_responses[b_root_ranks[0]].tensor_names().size());
          for (int i = 1; i < b_root_ranks.size(); i++) {
            allgather_response.add_ballgather_response(b_responses[b_root_ranks[i]]);
          }
          allgather_response.set_response_type(Response::BALLTOALL);
          allgather_response.set_entries_offsets();
          assert(allgather_response.entries_counts().size() == b_root_ranks.size());
          assert(allgather_response.root_ranks().size() == b_root_ranks.size());
          // can be gathered into recv fusion buffer
          int64_t total_tensor_size = 0;
          for (auto root_rank : b_root_ranks) {
            total_tensor_size += b_tensor_sizes[root_rank];
          }
          assert(total_tensor_size <= TensorFusionThresholdBytesCPU());
          response_list.add_response(allgather_response);
        }
      }
      // for (auto &response : b_responses) {
      //   if (response.tensor_names().size() > 0) {
      //     response_list.add_response(response);
      //   }
      // }
...
void InitializeHorovodOnce(const int* ranks, int nranks, int batchsize=0) {
  horovod_global.batch_size = batchsize;
  ...
  if (!horovod_global.initialize_flag.test_and_set()) {
    ...
    auto horovod_fusion_threshold = std::getenv(HOROVOD_CPU_FUSION_THRESHOLD);
    if (horovod_fusion_threshold != nullptr) {
      int64_t threshold = std::strtol(horovod_fusion_threshold, nullptr, 10);
      horovod_global.cpu_fusion_buffer_size = threshold;
    }
    std::cout << "cpu fusion threshold: " << horovod_global.cpu_fusion_buffer_size << "\n";
...
void horovod_init(const int* ranks, int nranks, int batch_size) {
  InitializeHorovodOnce(ranks, nranks, batch_size);
}
...
void FinishedQueueForGB::PushFTKsToQueue(std::string FTK){
  std::lock_guard<std::mutex> guard(mutex_);
  //gather task
  queue_.push(FTK);
}

std::vector<std::string> FinishedQueueForGB::GetFTKsFromQueue(){
  std::lock_guard<std::mutex> guard(mutex_);
  //gather task
...
Status EnqueueTensorGatherBcast(std::shared_ptr<OpContext> context,
                              std::shared_ptr<Tensor> tensor,
                              std::shared_ptr<Tensor> output,
                              std::shared_ptr<ReadyEvent> ready_event,
                              const std::string name, const int device,
  ...
  message.set_tensor_type(tensor->dtype());
  message.set_device(device);
  message.set_request_type(Request::GATHERBCAST);
  for (int i = 0; i < tensor->shape().dims(); ++i) {
    message.add_tensor_shape((int64_t)tensor->shape().dim_size(i));
  }
...
Status EnqueueTensorGather(std::shared_ptr<OpContext> context,
                              std::shared_ptr<Tensor> tensor,
                              std::shared_ptr<Tensor> output,
                              int root_rank,
                              std::shared_ptr<ReadyEvent> ready_event,
  ...
  message.set_tensor_type(tensor->dtype());
  message.set_device(device);
  message.set_request_type(Request::GATHER);
  message.set_batchid(batchid);
  message.set_root_rank(root_rank);
  if (num_elem > 0) { // for temporary variable like dgc gather
    message.add_tensor_shape((int64_t)num_elem);
  } else {
    for (int i = 0; i < tensor->shape().dims(); ++i) {
      message.add_tensor_shape((int64_t)tensor->shape().dim_size(i));
    }
...
Status EnqueueTensorBroadcast(std::shared_ptr<OpContext> context,
  ...
  message.set_batchid(batchid);
  if (num_elem > 0) {
    message.add_tensor_shape((int64_t)num_elem); // one dim
  } else {
    for (int i = 0; i < tensor->shape().dims(); ++i) {
      message.add_tensor_shape((int64_t)tensor->shape().dim_size(i));
    }
...
Status EnqueueTensorCBroadcast(std::shared_ptr<OpContext> context,
                              std::shared_ptr<Tensor> tensor,
                              std::shared_ptr<Tensor> output,
                              int root_rank,
                              std::shared_ptr<ReadyEvent> ready_event,
  ...
  message.set_root_rank(root_rank);
  message.set_device(device);
  message.set_request_type(Request::CBROADCAST); // broadcast the compressed data
  message.set_batchid(batchid);
  if (num_elem > 0) {
    message.add_tensor_shape((int64_t)num_elem); // one dim
  } else {
...
Status EnqueueTimestamp(const std::string tensor_name,
                        const std::string op_name,
                        const std::string args) {
  
  horovod_global.timestamp.Start(tensor_name, op_name, args);
