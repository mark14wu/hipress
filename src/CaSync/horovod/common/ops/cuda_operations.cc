...
Status CUDAAllreduce::FinalizeCUDAQueue(const std::vector<TensorTableEntry>& entries) {
  ...
  std::thread finalizer_thread([entries, first_entry, host_buffer,
    ...
    for (auto& e : entries) {
      ...
      timestamp.Start(e.tensor_name, "EALLREDUCE", std::to_string(e.batchid));
...
Status CUDAAllgather::FinalizeCUDAQueue(const std::vector<TensorTableEntry>& entries) {
  ...
  // TODO: use thread pool or single thread for callbacks
  std::thread finalizer_thread([entries, first_entry,
