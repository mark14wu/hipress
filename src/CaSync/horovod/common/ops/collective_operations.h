...
class GatherBcastOp : public HorovodOp {
public:
  GatherBcastOp(HorovodGlobalState* global_state);

  virtual ~GatherBcastOp() = default;
...
class GatherOp : public HorovodOp {
public:
  GatherOp(HorovodGlobalState* global_state);

  virtual ~GatherOp() = default;
  ...
  virtual bool Enabled(const ParameterManager& param_manager,
                       ...
                       const Response& response) const = 0;
protected:
  virtual void SetRecvcountsAndDisplcmnts(std::vector<TensorTableEntry>& entries, const Response& response,
...
public:
  ...
  virtual bool Enabled(const ParameterManager& param_manager,
                       ...
                       const Response& response) const = 0;
protected:
  virtual void SetRecvcountsAndDisplcmnts(std::vector<TensorTableEntry>& entries, const Response& response, const int roots_count,
                                          int64_t**& entry_component_sizes, int*& recvcounts, int*& displcmnts);

  virtual void SetSendcounts(std::vector<TensorTableEntry>& entries, const Response& response,
                                          const int root_rank, const int roots_index, int*& sendcounts);
