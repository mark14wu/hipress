...
Status NCCLAllreduce::Execute(std::vector<TensorTableEntry>& entries, const Response& response) {
  ...
  for (auto& name : response.tensor_names()) {
    global_state_->timestamp.Start(name, "BALLREDUCE", std::to_string(first_entry.batchid));
  }
...
NCCLAllgather::NCCLAllgather(NCCLContext* nccl_context,
                             MPIContext* mpi_context,
                             CUDAContext* cuda_context,
                             HorovodGlobalState* global_state)
...
Status NCCLAllgather::allocate_output(std::vector<TensorTableEntry>& entries, const Response& response) {
  for (size_t ec = 0; ec < entries.size(); ++ec) {
    ...
    int64_t one_entry_dimension_size = tensor_sizes[ec * global_state_->size];

    // Allgather output will have shape of:
    // (sum of first dimension of every tensor) x (tensor slice shape).
    TensorShape output_shape;
    output_shape.AddDim((int64_t) one_entry_dimension_size);
...
Status NCCLAllgather::Execute(std::vector<TensorTableEntry>& entries, const Response& response) {
  auto& first_entry = entries[0];
  ...
  Status status = allocate_output(entries, response);
  if (!status.ok()) {
    LOG(INFO) << "allocate output error";
    return status;
  }

  // Copy memory into the fusion buffer.
  if (entries.size() > 1) {
    LOG(INFO) << "allgather only have one entry per response";
  } else {
    send_buff = first_entry.tensor->data();
    recv_buff = (void*) first_entry.output->data();
  }

  // int64_t num_elements = first_entry.tensor->shape().num_elements();
  int64_t num_elements = 0;
  for (int i = 0; i < first_entry.tensor->shape().dims(); i++) {
    num_elements += first_entry.tensor->shape().dim_size(i);
  }
  num_elements *= global_state_->batch_size;

  // Do allreduce.
  auto nccl_result = ncclAllGather(send_buff, recv_buff,
                                   (size_t) num_elements,
                                   GetNCCLDataType(first_entry.tensor),
                                   *nccl_comm_, *stream_);
