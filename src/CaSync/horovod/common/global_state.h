...
using PartMessageTable  =std::unordered_map<
    std::string,
...
struct HorovodGlobalState {
  ...
  // Background thread running negotiation.
  std::thread background_thread;

  // Background thread perform the operations
  std::thread performoperation_thread;

  // A mutex that is used to visit the response queue
  std::mutex mutex_response_queue;

  // response queue for perform operation thread
  ...
  // Time point when last cycle started for negotiate
  std::chrono::steady_clock::time_point last_cycle_start;

  // Time point when last cycle started for perform operations
  std::chrono::steady_clock::time_point last_cycle_start_perform_operation;
  ...
  std::unique_ptr<PartMessageTable> partitioned_msg_table;

  // batch size for sufficient factors
  int batch_size = 0;

  // size of cpu fusion buffer in byte
  int64_t cpu_fusion_buffer_size = 67108864; // 64MB
  ...
  ~HorovodGlobalState() {
    ...
    if (performoperation_thread.joinable()) {
      shut_down = true;
      performoperation_thread.join();
    }
  }
};
