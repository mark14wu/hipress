...
const std::string& Request::RequestType_Name(RequestType value) {
  switch (value) {
    ...
    case RequestType::BROADCAST:
      ...
      return broadcast;
    case RequestType::GATHERBCAST:
      static const std::string gatherbcast("GATHERBCAST");
      return gatherbcast;
    case RequestType::GATHER:
      static const std::string gather("GATHER");
      return gather;
    case RequestType::CBROADCAST:
      static const std::string cbroadcast("CBROADCAST");
      return cbroadcast;
    default:
      static const std::string unknown("<unknown>");
      return unknown;
...
void Request::set_batchid(int32_t value) { batchid_ = value; }

int32_t Request::request_batchid() const { return batchid_; }
...
const std::string& Response::ResponseType_Name(ResponseType value) {
  switch (value) {
    ...
    case ResponseType::BROADCAST:
      static const std::string broadcast("BROADCAST");
      return broadcast;
    case ResponseType::CBROADCAST:
      static const std::string cbroadcast("CBROADCAST");
      return cbroadcast;
    case ResponseType::GATHERBCAST:
      static const std::string gatherbcast("GATHERBCAST");
      return gatherbcast;
    ...
    case ResponseType::ALLTOALL:
      ...
      return alltoall;
    case ResponseType::BALLTOALL:
      static const std::string balltoall("BALLTOALL");
      return balltoall;
...
void Response::add_allgather_response(const Response& response) {
  assert(response_type() == Response::ResponseType::ALLGATHER ||
         response_type() == Response::ResponseType::GATHER);
...
void Response::add_alltoall_response(const Response& response) {
  assert(response.response_type() == Response::ResponseType::GATHER);
  assert(response.tensor_names().size() >= 1);
  assert(response.devices() == devices());
  assert(response.root_ranks().size() == 1);
  for (auto name : response.tensor_names()) {
    add_tensor_name(name);
  }
  for (auto size : response.tensor_sizes()) {
    add_tensor_size(size);
  }
  add_root_rank(response.root_rank(0));
  add_entry_count(response.tensor_names().size());
}
// fuse multiple broadcast operators into one allgather operator
void Response::add_ballgather_response(const Response& response) {
  assert(response.response_type() == Response::ResponseType::CBROADCAST);
  assert(response.tensor_names().size() >= 1);
  assert(response.devices() == devices());
  assert(response.root_ranks().size() == 1);
  for (auto name : response.tensor_names()) {
...
const std::vector<int32_t>& Response::root_ranks() const {
  return root_ranks_;
}

void Response::add_entry_count(int32_t value) {
  entries_counts_.push_back(value);
}

const std::vector<int32_t>& Response::entries_counts() const {
  return entries_counts_;
}

void Response::add_entry_offset(int32_t value) {
  entries_offsets_.push_back(value);
}

const std::vector<int32_t>& Response::entries_offsets() const {
  return entries_offsets_;
}

void Response::set_entries_offsets() {
  int32_t offset = 0;
  for (auto count : entries_counts_) {
    entries_offsets_.push_back(offset);
    offset += count;
  }
}

void Response::set_entries_counts(const std::vector<int32_t>& value) {
  entries_counts_ = value;
}

void Response::set_entries_offsets(const std::vector<int32_t>& value) {
  entries_offsets_= value;
}

void Response::set_root_ranks(const std::vector<int32_t>& value) {
  root_ranks_ = value;
}
...
void Response_ParseFromWire(Response& response,
  ...
  response.set_entries_counts(std::vector<int32_t>(obj->entries_counts()->begin(),
                                                   obj->entries_counts()->end()));
  response.set_entries_offsets(std::vector<int32_t>(obj->entries_offsets()->begin(),
                                                    obj->entries_offsets()->end()));
  response.set_root_ranks(std::vector<int32_t>(obj->root_ranks()->begin(),
                                               obj->root_ranks()->end()));
}
...
void Response_SerializeToWire(const Response& response,
  ...
  auto entries_counts_wire = builder.CreateVector(response.entries_counts());
  auto entries_offsets_wire = builder.CreateVector(response.entries_offsets());
  auto root_ranks_wire = builder.CreateVector(response.root_ranks());
  ...
  response_builder.add_entries_counts(entries_counts_wire);
  response_builder.add_entries_offsets(entries_offsets_wire);
  response_builder.add_root_ranks(root_ranks_wire);
...
void Response::clear() {
  response_type_ = ResponseType::ALLREDUCE;
  tensor_names_.clear();
...
void Response::set_response(const Response& response) {
  set_response_type(response.response_type());
  set_tensor_names(response.tensor_names());
...
void Response::add_root_rank(int32_t value) {
  root_ranks_.push_back(value);
}

int32_t Response::root_rank(int32_t index) const {
  assert(index < root_ranks_.size());
  return root_ranks_[index];
}
