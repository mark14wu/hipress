...
public:
  enum RequestType {
    ALLREDUCE = 0, ALLGATHER = 1, BROADCAST = 2, GATHERBCAST = 3, GATHER = 4, CBROADCAST = 5,
  };
  ...
  void set_batchid(int32_t value);

  int32_t request_batchid() const;
...
public:
  enum ResponseType {
    ALLREDUCE = 0, ALLGATHER = 1, BROADCAST = 2, ERROR = 3, GATHERBCAST = 4, GATHER = 5, CBROADCAST = 6, ALLTOALL = 7, BALLTOALL = 8,
  };
  ...
  // fuse multiple gather response into one alltoall response
  void add_alltoall_response(const Response& response);
  // fuse multiple bcast response into one allgather response
  void add_ballgather_response(const Response& response);
  ...
  void add_entry_offset(int32_t value);
  const std::vector<int32_t>& entries_offsets() const;
  void set_entries_offsets();
  void set_entries_offsets(const std::vector<int32_t>& value);
...
private:
  ...
  std::vector<int32_t> entries_offsets_;
