...
def _gather_function_factory(tensor):
    return 'horovod_torch_gather_async_' + tensor.type().replace('.','_')

def _cbroadcast_function_factory(tensor):
    return 'horovod_torch_cbroadcast_async_' + tensor.type().replace('.','_')
...
def _gather_async(output, tensor, root_rank, num_elem=0, name=None, batchid=0):
    function = _check_function(_gather_function_factory, tensor)
    handle = getattr(mpi_lib, function)(
        tensor, output, root_rank, num_elem, batchid,
        name.encode() if name is not None else _NULL
    )
    _gather_handle_map[handle] = (tensor, output)
    return handle

def _cbroadcast_async(tensor, root_rank, num_elem=0, name=None, batchid=0):
    #tensor is output
    function = _check_function(_cbroadcast_function_factory, tensor)
    handle = getattr(mpi_lib, function)(
        tensor, tensor, root_rank, num_elem, batchid,
        name.encode() if name is not None else _NULL
    )
    _cbroadcast_handle_map[handle] = (tensor, tensor)
    return handle

cbroadcast_async_ = _cbroadcast_async
...
# def gather_async(**d):
#     return _gather_async()

# def gather_async(output, tensor, dep, root_rank, num_elem=0, name=None, batchid=0):
#     return ...
gather_async_ = _gather_async
...
def get_all_get_all_finished_keys(task_id):
    if task_id != 10 and task_id != 20 and task_id != 30:
        raise(ValueError)
    return mpi_lib.get_all_finished_keys(task_id)
...
def gather_synchronize(handle):
    if handle not in _gather_handle_map:
        return
    mpi_lib.horovod_torch_wait_and_clear(handle)
    _, output = _gather_handle_map.pop(handle)
    return output

def cbroadcast_synchronize(handle):
    if handle not in _cbroadcast_handle_map:
        return
    mpi_lib.horovod_torch_wait_and_clear(handle)
    _, output = _cbroadcast_handle_map.pop(handle)
    return output