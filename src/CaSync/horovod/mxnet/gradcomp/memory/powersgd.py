...
class PowersgdMemory(Memory):
    ...
    def initialize(self, index, grad, rank):
        # view grad as 2-D matrix(M) with square shape
        interval = math.ceil(math.sqrt(grad.size))
        self._p_memory[index] = mx.nd.zeros(shape=(interval, rank), dtype=grad.dtype, ctx=grad.context)
        self._q_memory[index] = mx.nd.normal(shape=(interval, rank), dtype=grad.dtype, ctx=grad.context)
