...
opt = mx.optimizer.create(args.optimizer, **trainer_params)
...
def train():
    ...
    if parameters is not None:
        hvd.broadcast_parameters(parameters, root_rank=0)
    param_dict_avg = None
    t = 0
    avg_trigger = 0
    n = 5
    ...
    for epoch in range(args.epochs):
        ...
        while i < len(train_data) - 1 - 1:
            ...
            trainer.update(1, allreduce=True)
            ...
            # trainer.step(1)
            trainer.update(1, allreduce=False)
            ...
            if batch_i % args.log_interval == 0 and batch_i > 0:
                ...
                if rank == 0:
                    batch_speed = hvd_size * args.batch_size * args.log_interval / (time.time() - start_log_interval_time)
                    logging.info('[Epoch %d Batch %d/%d] current loss %.2f, ppl %.2f, '
                        'throughput %.2f samples/s, lr %.2f', epoch, batch_i, len(train_data) // args.bptt, cur_L, math.exp(cur_L),
                            batch_speed,
        ...
        mx.nd.waitall()
        if rank == 0:
            epoch_speed = (hvd_size * args.batch_size * (batch_i+1)) / (time.time() - start_epoch_time)
            logging.info('[Epoch %d] throughput %.2f samples/s', 
                epoch, (hvd_size * args.batch_size * (batch_i+1)) / (time.time() - start_epoch_time))
            logging.info('[Epoch %d] Average Speed %.2f', epoch, epoch_speed)
