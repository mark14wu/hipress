...
class trainingApp:
	...
	def training(self, parsedArgs):
		cmd = '''./%(horo)s.sh %(numproc)d %(serverset)s %(batches)d %(blk)d %(alg)s %(thr)d 2>&1 | tee %(path)s/language_model_ringtrain_%(time)s.log'''%(
			{
...
def endOfonce(resultpath, parsedArgs):
	...
	for hostid in hostset:
		hostname = 'egpu' + str(hostid)
		cmd = r'''ssh %s "kill \$(ps -aux | grep word_language | awk '{print \$2}')"'''%(hostname)
		print(cmd)
		os.system(cmd)
	if parsedArgs.profiler:
		for hostid in hostset:
			hostname = 'egpu' + str(hostid)
			cmd = "scp gpu@%s:/home/gpu/mxnet/horovod/examples/language_model/horovod-mxnet/*.json %s"%(hostname, resultpath)
			print(cmd)
			os.system(cmd)
			cmd = "ssh %s 'rm -f /home/gpu/mxnet/horovod/examples/language_model/horovod-mxnet/*.json'"%(hostname)
			print(cmd)
			os.system(cmd)
...
if __name__ == '__main__':
	...
	# initialize()
	applications = ['standard-LSTM']
	for app in applications:
		dirname = str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()))
		if parsedArgs.extra:
			dirname = dirname + parsedArgs.extra
		basepath = '/home/gpu/mxnet/horovod/horovod-mxnet/trainResult/%s/%dmachines-%s/%s'%(app, len(parsedArgs.servers.split(',')), parsedArgs.numbatches, dirname)
