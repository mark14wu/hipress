...
import horovod.mxnet as hvd # Horovod
import socket
...
lr = args.lr * hvd_size # Horovod: schedule the lr with number of workers
...
ctx = mx.cpu(local_rank) if args.gpu is None else mx.gpu(args.gpu)
...
# load checkpointing
if args.gpu:
    output_dir = args.output_dir if hvd.rank() == 0 else os.path.join(args.output_dir, str(hvd.rank()))
else:
...
logging.info('processing dataset... time: %.2f', time.time())
if task.task_name == 'MNLI':
...
def train(metric):
    if args.num_batches > len(train_data):
        logging.info('there is no so much data in train_data')
        exit
    if args.profile:
        mx.profiler.set_config(filename='profile_Bert_{}_{}.json'.format(socket.gethostname(), args.task_name), profile_all=True)
    ...
    try:
        ...
        logging.info('get the trainer successful! ready to train, time: %.2f', time.time())
    ...
    for epoch_id in range(args.epochs):
        ...
        for batch_id, seqs in enumerate(train_data):
            if batch_id+1 > args.num_batches:
                break
            ...
            if not accumulate or (batch_id + 1) % accumulate == 0:
                trainer.allreduce_grads() # reduce gradients from devices
                trainer.update(accumulate if accumulate else 1, allreduce=True) # reduce gradients from machines
                nlp.utils.clip_grad_global_norm(params, 1) # clip gradients
                trainer.update(accumulate if accumulate else 1, allreduce=False) # update weight
            ...
            if (batch_id + 1) % (args.log_interval) == 0:
                ...
                if not isinstance(metric_nm, list):
                    ...
                    metric_val = [metric_val]
                if rank == 0:
                    batch_speed = hvd_size * batch_size * args.log_interval / (time.time() - btic)
                    eval_str = '[Epoch %d Batch %d/%d] loss=%.4f, lr=%.7f, speed=%.2f, metrics=' + \
                        ','.join([i + ':%.4f' for i in metric_nm])
                    logging.info(eval_str, epoch_id + 1, batch_id + 1, len(train_data),
                             step_loss / args.log_interval,
                             trainer.learning_rate, batch_speed, *metric_val)
                btic = time.time()
                step_loss = 0
        mx.nd.waitall()
        elapsed_time = time.time() - tic
        if rank == 0:
            epoch_speed = hvd_size * batch_size * (batch_id+1) / elapsed_time
            logging.info('Epoch[%d]\tAverage Speed: %.2f', epoch_id, epoch_speed)
        ...
        tic = toc
        if epoch_id == args.epochs and rank == 0:
            logging.info("end_of_once_training")
    if args.profile:
