...
flags.DEFINE_integer('threshold', 262144,'compress threshold, number of floats'
                     'greater than threshold ared updated using compression')
...
flags.DEFINE_string('benchmark_test_id', None,
                    ...
                    'system.')
flags.DEFINE_string('comp_alg', 'dgc', 'compression algorithms')
  ...
  def add_forward_pass_and_gradients(self,
    ...
    def forward_pass_and_gradients():
      ...
      if self.params.variable_update == 'horovod':
        ...
        logging.fatal(cf.blue("zhouquan: run over here"))
        gc = hvd.Gather_CBroadcast(self.params.threshold)
        # gc = hvd.Gather_CBroadcast(268435456)
        if self.params.horovod_device:
          horovod_device = '/%s:0' % self.params.horovod_device
        else:
          horovod_device = ''
        # All-reduce gradients using Horovod.
        grads = [gc.run(grad, average=False, device_dense=horovod_device)
                 for grad in grads]
