...
parser.add_argument('--print-intervals', type=int, default=100,
                    help = "print average speed per interval epochs")
parser.add_argument('--threshold', type=int, default=65536,
...
print("iterations:",args.num_iterations)
...
def train(epoch):
    ...
    with tqdm(total=len(train_loader),
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            now_time = time.time()
            global last_time
            speed = hvd.size() * len(data) / (now_time - last_time)
            last_time = now_time
            t.set_postfix({'loss': train_loss.avg.item(),
                           'accuracy': 100. * train_accuracy.avg.item(),
                           'Speed': '{:.2f} samples/sec'.format(speed)})
            t.update(1)
            if (batch_idx + 1) % args.print_intervals == 0:
                average_end = time.time()
                average_speed = hvd.size() * args.batch_size * args.print_intervals / (average_end - average_start)
                average_start = average_end
                print("Average Speed: {:.2f} samples/sec".format(average_speed))
